...
using Oceananigans.Grids: VerticallyStretchedRectilinearGrid, xnodes, znodes, topology, with_halo
...
using Oceananigans.ImmersedBoundaries: ImmersedBoundaryGrid # Import ImmersedBoundaryGrid
using Oceananigans.Fields: Field # Needed to create bottom_height field

using ..Parameters: CutCellParameters # Access the struct (now only physical params)
using ..Geometry: bathymetry_profile, bathymetry_derivative, CutCellBottom # Need geometry functions and new IB type
...
function build_cut_cell_model(params::CutCellParameters) # Only physical params are passed here
    ...
    # Create the UNDERLYING RectilinearGrid
    underlying_grid = VerticallyStretchedRectilinearGrid(
        topology = (Bounded, Flat, Bounded),
        size = (params.nx, 1, params.nz), # Explicitly add Ny=1
        x = params.x_domain,
        y = (0.0, 1.0), # Explicitly add y domain
        z_faces = z_faces_func.(0:params.nz),
        halo = (2, 2, 2) # 2 in x, y, z
    )
    @info "Underlying grid created: $(underlying_grid)"

    # --- Immersed Boundary Setup ---

    # Create a Field for the bottom_height (negative of bathymetry depth)
    # Bathymetry is defined at C,C locations.
    bottom_height_field = Field{Center, Center, Nothing}(underlying_grid)
    # Use the bathymetry_profile function to set the height at C,C points
    # bathymetry_profile returns positive depth. We need negative height.
    @inbounds for i in 1:underlying_grid.Nx, j in 1:underlying_grid.Ny
        x_c = xnodes(Center, underlying_grid)[i] # Get x coordinate at C
        bottom_height_field[i, j, 1] = -bathymetry_profile(x_c) # Store negative height
    end

    # Create the CutCellBottom immersed boundary object
    # minimum_fractional_Δz can be adjusted
    immersed_boundary = CutCellBottom(bottom_height_field; minimum_fractional_Δz=0.1)

    # Create the ImmersedBoundaryGrid
    grid = ImmersedBoundaryGrid(underlying_grid, immersed_boundary)
    @info "ImmersedBoundaryGrid created: $(grid)"
    ...
    # Boundary conditions: Solid wall (zero velocity, zero flux) on sides (x) and bottom (z) are implicitly handled by IBG where solid.
    # Top (z=0) BC for u: Wind stress applied as a kinematic flux.
    wind_stress_tau_x = 0.05 # Get from params? Add to struct? (currently hardcoded in script)
                             # For now, keep hardcoded or pass via a different parameters mechanism if not in CutCellParameters.
                             # Let's add wind_stress_tau_x to CutCellParameters struct.
    wind_stress_flux = wind_stress_tau_x / params.rho0 # Use rho0 from params

    # Define BCs on the *underlying* grid. IBG applies them where the boundary is not immersed.
    # Lateral BCs (x): Solid Wall is default for Bounded, enforced by IBG on boundary walls.
    # Bottom BC (z): Solid Wall is default for Bounded, enforced by IBG on bottom boundary.
    # Top BC (z=0): Wind stress for u, default (typically zero flux) for tracers and v.
    u_bcs = FieldBoundaryConditions(grid.underlying_grid, top = FluxBoundaryCondition(wind_stress_flux))

    # Note: IBG should handle BCs at immersed solid faces/cells based on the specified BCs
    # on the underlying grid. Default is often zero flux/velocity.
    # Forcings also manually zero out tendency at immersed points, providing a second layer of safety.

    model = HydrostaticFreeSurfaceModel(
        grid = grid, # Use the ImmersedBoundaryGrid
        ...
        tracers = (:T, :S),
        parameters = (; cut_cell_params = params), # Pass the physical parameters struct
    ...
    # Note: These are defined on the *ImmersedBoundaryGrid*. Setting values in immersed cells
    # will be overwritten or not used by forcings/diagnostics due to is_immersed checks.
    ...
    # Set initial temperature and salinity on the IBG
    set!(model, T=T_init, S=S_init)

    # Initialize velocities to zero
    set!(model, u=0.0, v=0.0, w=0.0) # Model w field is updated by diagnostic

    # IBG automatically handles setting velocity/tracer to 0 in solid regions for many operations.
    # Forcings also explicitly check `!is_immersed` to avoid calculations in solid regions.


    # The w diagnostic calculates w *after* the state update using u, then stores it in cut_cell_w.
    # This diagnostic field needs to be created on the ImmersedBoundaryGrid.
